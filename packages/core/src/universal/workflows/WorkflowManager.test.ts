...
class MockPlanner implements IWorkflowPlanner {
  ...
  getCapabilities() {
    return {
      supportedComplexity: ['simple', 'medium', 'complex'] as const,
...
describe('WorkflowManager', () => {
  ...
  describe('initialization', () => {
    ...
    it('should use provided planner and executor', () => {
      expect(manager.getPlanner()).toBe(mockPlanner);
      expect(manager.getExecutor()).toBe(mockExecutor);
    });
  });
  ...
  describe('workflow control', () => {
    it('should pause workflow', async () => {
      const execution = await manager.createAndExecute(mockRequest, mockAnalysis, mockAgents);
      const paused = await manager.pauseWorkflow(execution.id);
    ...
    it('should resume workflow', async () => {
      const execution = await manager.createAndExecute(mockRequest, mockAnalysis, mockAgents);
      await manager.pauseWorkflow(execution.id);
      const resumed = await manager.resumeWorkflow(execution.id);
    ...
    it('should cancel workflow', async () => {
      const execution = await manager.createAndExecute(mockRequest, mockAnalysis, mockAgents);
      const cancelled = await manager.cancelWorkflow(execution.id);
    ...
    it('should get workflow status', async () => {
      const execution = await manager.createAndExecute(mockRequest, mockAnalysis, mockAgents);
      const status = await manager.getWorkflowStatus(execution.id);
  ...
  describe('workflow history and metrics', () => {
    it('should track workflow history', async () => {
      ...
      await manager.cancelWorkflow(execution.id);
  ...
  describe('workflow cleanup', () => {
    it('should clean up completed workflows', async () => {
      ...
      await cleanupManager.cancelWorkflow(execution.id);
